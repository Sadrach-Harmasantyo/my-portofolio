...
const workExperiences = [
  {
    ...
    responsibilities: [
      "Contributed to chatbot development on the Netmonk AI & Data Team.",
      "Developed 5+ features, fixed 15+ bugs, and improved unit test coverage to more than 80%.",
      "Utilized modern technologies including React.js, Nuxt.js, TypeScript, Python, FastAPI, PostgreSQL, and MongoDB.",
    ],
  },
  {
...
export default function WorkExperience() {
  ...
  return (
    <section id="experience" className="py-16 lg:py-24" ref={containerRef}>
      <div className="container">
        <SectionHeader eyebrow="My Career Journey" title="Work Experience" description="A timeline of my professional roles and contributions." />
        ...
        <div className="relative mt-10 md:mt-20">
          ...
          >
            ...
            <motion.div className="absolute top-0 w-full bg-gradient-to-b from-emerald-300 to-sky-400 rounded-full" style={{ height: lineHeight }} />
          </div>
            ...
            {workExperiences.map((exp, index) => {
              return (
                <div key={index} className="relative flex flex-col md:flex-row items-start md:items-center w-full">
                  ...
                  >
                    <h3 className="font-serif text-2xl lg:text-3xl text-emerald-300">{exp.role}</h3>
                    <div className="inline-flex font-bold uppercase tracking-widest text-sm text-white mt-2">{exp.company}</div>
                    <div className="text-white/60 text-sm mt-4 font-semibold flex items-center md:justify-end gap-2">
                      <span>{exp.duration}</span>
                    </div>
                    <div className="text-white/50 text-sm mt-1">{exp.location}</div>
                  </motion.div>
                    ...
                    <Card className="p-6 md:p-8 hover:-translate-y-2 hover:shadow-2xl hover:shadow-emerald-300/10 transition-all duration-500 group relative z-10 w-full overflow-hidden">
                      ...
                      <ul className="flex flex-col gap-3 relative z-20">
                        {exp.responsibilities.map((resp, idx) => (
                          <li key={idx} className="flex gap-2 text-sm md:text-base text-white/50 group-hover:text-white/70 transition-colors duration-300">
